...
while [ $continue -eq 1 ]; do
	...
	cauA=`cat cauhoi.txt | head -$(($current+1)) | tail -1 | cut -d'|' -f3`
	cauB=`cat cauhoi.txt | head -$(($current+1)) | tail -1 | cut -d'|' -f4`
	cauC=`cat cauhoi.txt | head -$(($current+1)) | tail -1 | cut -d'|' -f5`
	cauD=`cat cauhoi.txt | head -$(($current+1)) | tail -1 | cut -d'|' -f6`
	echo "A: $cauA"
	echo "B: $cauB"
	echo "C: $cauC"
	echo "D: $cauD"
